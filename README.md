...
- Catálogo com 29 produtos em 7 categorias, todos com preço e ficha técnica reais (exceto os 2 kits prontos, que são orçamento real da própria Marques): **kits prontos** (2), **painéis solares** (5), **inversores** (6), **cabos e conectores** (6), **parafusos e estrutura** (6, vendida peça a peça — trilho, terminal, parafuso — como o mercado realmente vende), **baterias** (2) e **controlador de carga** (2). Painéis/inversores/baterias/controlador/cabos/estrutura usam preço de um concorrente (Apex Energia Solar, cotado em dezembro/2026) como referência de mercado até a Marques definir fornecedor próprio.
...
- **Estimativa de frete por zona no checkout**, no lugar do texto fixo "A combinar": ao preencher o CEP, calcula a UF (via ViaCEP, que já era usado pro endereço) e mostra uma faixa de preço (Cuiabá-MT/região, estados vizinhos, demais estados). Continua deixando claro que é estimativa e o valor exato é confirmado pela equipe — não tem integração real de transportadora. *Tentamos usar o calculador de frete da Apex como referência, mas ele está com bug no site deles (não retorna resultado nem com CEP válido) — não tinha o que copiar.*

### ⚠️ Decisões/contas que só você pode resolver
1. **Fornecedor e margem definitivos do catálogo** — painéis, inversores, baterias, controlador, cabos e estrutura já têm preço/ficha técnica reais (referência: Apex Energia Solar, concorrente em Cuiabá-MT), mas ainda é preço de um concorrente, não o fornecedor/margem que a Marques vai efetivamente usar. Falta decidir isso e trocar as fotos de estoque por fotos próprias (ver item 9).
...
6. **Frete ainda é estimativa, não cálculo real** — o checkout mostra uma faixa de preço por zona (`estimarFrete()` em `app.js`). Testamos direto a API dos Correios: o endpoint público antigo (`CalcPrecoPrazo`) está fora do ar (dá timeout, não responde mais pra ninguém — foi descontinuado há anos); a API nova oficial (`api.correios.com.br`) exige login/senha de um contrato ativo com "cartão de postagem" — não dá pra usar sem a Marques abrir uma conta empresarial nos Correios. Caminho realista: contratar uma API de frete que já resolve isso por trás (Melhor Envio, Frenet etc. — cadastro simples, usam o contrato deles com os Correios) e alimentar peso/cubagem real de cada produto — outro projeto à parte.
...
9. **Fotos dos produtos: só 3 de 29 itens têm foto real hoje.** A do inversor Solis (kits) bate com o modelo exato (S6-GR1P3K-M); o painel TSUN 630W (kits + `pn3`) também é exato; o `pn2` (TSUN 600W) reaproveita a mesma foto por ser da mesma linha ("RIO" bifacial preta), só que variante diferente. Os outros 26 itens (demais painéis, todos os inversores, baterias, controlador, cabos, estrutura) ainda caem no ícone-padrão da categoria — tentamos buscar fotos oficiais de fabricante (Jinko, Canadian Solar, Fronius, ZTROON etc.) pra completar, mas os sites deles têm muita fricção pra automação (JS pesado, catálogo em PDF, paywall de imagem em banco de fotos) e não valia a pena adivinhar imagem errada pra um produto. Fica como trabalho manual: baixar do site oficial de cada fabricante (ou pedir direto ao distribuidor/fornecedor que a Marques fechar) e colocar em `assets/products/`, referenciando no campo `image` de cada item em `app.js`.
