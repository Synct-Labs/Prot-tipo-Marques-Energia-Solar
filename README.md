...
6. **Frete ainda é estimativa, não cálculo real** — o checkout agora mostra uma faixa de preço por zona (`estimarFrete()` em `app.js`), mas não integra com transportadora/Correios. Pra ter valor exato na hora, precisa contratar uma API de frete (Melhor Envio, Frenet etc.) e alimentar peso/cubagem real de cada produto — outro projeto à parte.
...
9. **Fotos dos produtos (`assets/products/` e os "IMAGEM ILUSTRATIVA" do catálogo) são de sites de revenda ou nem existem ainda** — a do inversor Solis bate com o modelo exato (S6-GR1P3K-M); a do módulo TSUN é da variante de 620W da mesma linha "RIO" bifacial preta (não achamos foto de revenda específica da variante 630W). Os itens novos (painéis, inversores, baterias, controlador, cabos, estrutura) ainda não têm foto nenhuma. Vale confirmar com o fornecedor/distribuidor se pode usar essas imagens comercialmente, ou pedir fotos oficiais direto do fabricante.
